...
import { LayoutDashboard, User, ShoppingCart, Settings, LogOut, List, X } from 'lucide-react';
...
const FarmerSidebar = ({ currentPage, setCurrentPage, onLogout, isSidebarOpen, setIsSidebarOpen }: FarmerSidebarProps) => {
  const navItems = [
    { id: 'farmer_dashboard', label: 'My Dashboard', icon: LayoutDashboard },
    { id: 'farmer_profile', label: 'Update Profile', icon: User },
    { id: 'farmer_listings', label: 'My Listings', icon: List },
    ...
    <>
      ...
      <div className="flex-1 flex flex-col justify-between overflow-y-auto">
        ...
        <div className="px-4 py-4 border-t dark:border-gray-700">
            <a 
              href="#" 
              onClick={(e) => { e.preventDefault(); handleLinkClick('farmer_settings'); }} 
              className={classNames(
                 'flex items-center px-4 py-3 mt-2 text-gray-600 rounded-lg hover:bg-primary-50 hover:text-primary-700 transition-colors duration-200 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-white',
                currentPage === 'farmer_settings' ? 'bg-primary-100 text-primary-700 font-semibold dark:bg-gray-700 dark:text-white' : 'hover:bg-gray-100'
              )}
            >
