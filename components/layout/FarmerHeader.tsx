...
const FarmerHeader = ({ farmerName, currentPage, onMenuClick, theme, setTheme }: FarmerHeaderProps) => {
  const pageTitles: Partial<Record<Page, string>> = {
    farmer_dashboard: 'My Dashboard',
    farmer_profile: 'Update My Profile',
    farmer_listings: 'My Produce Listings',
    marketplace: 'Farmers Marketplace',
    farmer_settings: 'Settings',
    ...
    <header className="flex-shrink-0 flex items-center justify-between h-20 px-4 md:px-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
      ...
      <div className="flex items-center">
        ...
        <div className="ml-2 md:ml-4 flex items-center">
            <div className="h-10 w-10 rounded-full bg-primary-200 dark:bg-primary-800/50 flex items-center justify-center flex-shrink-0">
                <User className="h-6 w-6 text-primary-700 dark:text-primary-300"/>
            </div>
          <div className="ml-2 hidden md:block">
            <p className="text-sm font-medium text-gray-700 dark:text-gray-200">{farmerName}</p>
